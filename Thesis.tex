\documentclass[12pt, a4paper, twoside, pdftex, cleardoublepage=empty, numbers=noenddot]{scrbook}
% 12pt                  => Grundschrift, Papiergröße, beidseitiger Druck,
% cleardoublepage=empty => leere Seite ohne Kopfzeile,
% numbers=noenddot      => Nummerierung ohne Punkt am Schluss
...
% GEÄNDERT: color durch xcolor ersetzt, da tikz xcolor lädt und sonst ein Optionskonflikt entsteht
...
\usepackage{epstopdf}
\usepackage{float}                 % NEU: für Gleitobjekte mit Option [H] (vorher Fehler 'Unknown float option H')
\usepackage{booktabs}              % NEU: Linien in Tabellen (Vergleichs- und Zuordnungstabellen)
\usepackage{array}                 % NEU: linksbündige Tabellenspalten (>{\raggedright\arraybackslash})
%\usepackage{natbib}
...
\frontmatter
  ...
  % Passendes Deckblatt auswählen und anpassen
  \input{00_1_Titel/MSc_Thesisdeckblatt.tex}
  % \input{00_1_Titel/MSc_Thesisdeckblatt_EN.tex}
  % \input{00_1_Titel/BSc_Thesisdeckblatt.tex}
  % \input{00_1_Titel/BSc_Thesisdeckblatt_EN.tex}
  \clearpage
  \input{00_2_Erklaerung}
  \clearpage
  \input{00_3_Abstract}
  \clearpage
  ...
  \listoftables           % NEU: Tabellenverzeichnis, da neue Tabellen hinzukommen

\mainmatter

  \input{01_Einleitung}
  \input{02_Forschungsstand}
  \input{03_Ansatz}
  \input{04_Implementierung}
  \input{05_Validierung}
  \input{06_ZusammenfassungAusblick}
...
\backmatter
  ...
  \bibliography{Literaturverzeichnis,manual-refs}{}
